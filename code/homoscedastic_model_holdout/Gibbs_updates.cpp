...
// // [[Rcpp::depends(RcppArmadillo)]]
// // [[Rcpp::export]]
// double Z1_update(int s_ind, int t_ind, List Z1,List X, List L1,mat bet1,mat gam1, double a_11, 
//                 vec V1,double sig21, int n_lag,IntegerVector lags){
//   
//   int s_idx = s_ind - 1;
//   int t_idx = t_ind - 1;
//   
//   rowvec b1 = bet1.row(s_idx) ;
//   rowvec g1 = gam1.row(s_idx) ;
//   vec Z_temp = Z1[s_idx];
//   mat X_temp = X[s_idx];
//   mat L_temp = L1[s_idx];
//   
//   
//   double V_temp = V1[s_idx] * a_11;
//   
//   double m_new = (sum(b1 * X_temp.row(t_idx).t() + g1 * L_temp.row(t_idx).t()) + V_temp) / sig21;
//   double v_inv_new = 1 / sig21 ;
//  //  Rcpp::Rcout << "m * V = "<< m_new * sig21<< std::endl;
// 
//  if(n_lag != 0){
//    for(int i = 0; i < n_lag ; i++){
//      int t_idx_new = t_idx + lags[i] ;
//      m_new += g1[i] * (Z_temp[t_idx_new] - sum(b1 * X_temp.row(t_idx_new).t() + V_temp)) / sig21;
...
// // [[Rcpp::depends(RcppArmadillo)]]
// // [[Rcpp::export]]
// double Z2_update(int s_ind, int t_ind, List Z2,List X, List L2,mat bet2,mat gam2, double a_12, 
//                  vec V1,vec V2,double sig22, int n_lag,IntegerVector lags){
//   
//   int s_idx = s_ind - 1;
//   int t_idx = t_ind - 1;
//   
//   rowvec b1 = bet2.row(s_idx) ;
//   rowvec g1 = gam2.row(s_idx) ;
//   vec Z_temp = Z2[s_idx];
//   mat X_temp = X[s_idx];
//   mat L_temp = L2[s_idx];
//   
//   
//   double V_temp = V1[s_idx ] * a_12 + V2[s_idx];
//   
//   double m_new = (sum(b1 * X_temp.row(t_idx).t() + g1 * L_temp.row(t_idx).t()) + V_temp) / sig22;
//   double v_inv_new = 1 / sig22 ;
//   //Rcpp::Rcout << "m * V = "<< m_new * sig22<< std::endl;
//   
//   if( n_lag != 0){
//     for(int i = 0; i < n_lag ; i++){
//       int t_idx_new = t_idx + lags[i] ;
//       vec L_row = L_temp.row(t_idx_new).t();
//       m_new += g1[i] * (Z_temp[t_idx_new] - sum(b1 * X_temp.row(t_idx_new).t() + 
//         g1 * L_row + V_temp  - g1[i] * L_row[i] )) / sig22;
