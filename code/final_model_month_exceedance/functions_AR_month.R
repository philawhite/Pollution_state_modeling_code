...
MCMC_mexico = function(dat,dat_dec ,lags1,lags2 ,reps ,burn ,month,core_use = 1,thin = 1){
  
  dat_dates = as.Date(dat$Date,format = "%m/%d/%Y")
  mon = month(dat_dates)
  
  dat_full = dat
  idx_train = which(mon < month)
  idx_all = which(mon <= month)
  idx_test = which(mon == month)  
  
  dat = dat_full[idx_train,]
  ns = length(unique(dat$loc_ind))
  nt = length(unique(dat$time_ind))
  ...
  preds = mclapply( 1:(reps/thin) , function(i){
    out = sapply(idx_test,function(xx){
  ...
  return(list(sig21 = sig21, sig22 = sig22,bet1 = bet1,bet2 = bet2,bet10 = bet10,bet20 = bet20,
              ...
              tau21 = tau21,tau22 = tau22,a12 = a12 ,preds = preds))
  
}


################################################################################################
################# Phase and exceedance stuff Monthly - Conditional  ############################
################################################################################################
...
Phase_MC = function(dat,preds,ns,month,L1_o= 154,L2_o = 204,L1_p = 214,L2_p = 354){
  ...
  pred_time = dat_test$time_ind
  t_un = unique(dat_test$time_ind)
  
  nt = length(unique(dat_test$time_ind))
  ...
  O3_now = sapply(t_un,function(x){preds[1,dat_test$time_ind == x]})
...
prob_phase = function(phase,nr = nr,reps = reps){
  ...
  nt = dim(phase)[2]
  reg_day = array(0,c(nr,nt/24,3))
  reg_hour = array(0,c(nr,nt,3))
  
  tot_day = matrix(0,nrow = nt/24,3)
  ...
  for(i in 1:(nt/24)){
    temp = apply(phase[,((i-1)*24 + 1):(i*24),],c(1,3),max)
...
prob_exceed = function(phase_O3,phase_PM10,nr = nr,reps = reps){
  ...
  reg_O3_day = matrix(0,ncol = nr, nrow = nt/24 )
  reg_PM10_day = matrix(0,ncol = nr, nrow = nt/24 )
  reg_O3_hour = matrix(0,ncol = nr, nrow = nt )
  reg_PM10_hour = matrix(0,ncol = nr, nrow = nt )
  
  tot_O3_day = numeric(nt/24)
  tot_PM10_day = numeric(nt/24)
